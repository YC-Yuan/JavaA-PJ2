    ...
    private void fightWith(Player player) {
        while (hp > 0 & player.getHealth() > 0) {
            if (chanceGenerate(criticalChance)) {//勇者攻击怪物被闪避
                dodgeTimes++;
                Game.addDisplayText(name + "发动了闪避，无视勇者攻击！" + name + "还剩" + hp + "点生命值");
            }
            else {//勇者正常攻击怪物
            ...
            if (chanceGenerate(criticalChance)) {//怪物攻击勇者造成暴击
                ...
                Game.addDisplayText(name + "发动暴击，对勇者造成" + Math.max(0,2 * (atk - player.getDefence())) + "点伤害！" + "勇者还剩" + player.getHealth() + "点生命值");
            }
            else {//怪物正常攻击勇者
        ...
        Game.addDisplayText(name + "共触发了" + criticalTimes + "次暴击和" + dodgeTimes + "次闪避");
        fightEnd(player);
    }
